...
# 1. File Paths
DATA_FILENAME = "enrichmentrundata.csv"      # Name of your .csv file
DATA_FOLDER = Path("data/")        # Folder where csv is located
FIGURES_FOLDER = Path("figures/")  # Folder to save the image
OUTPUT_NAME = "lithium_enrichment_sweep"  # Name of the saved image file

# 2. Data Loading Settings
DELIMITER = ","                    # Separator (use ',' for csv, '\t' for tab-separated)
SKIP_HEADER = 2                   # Number of header rows to skip in the CSV file

# 3. Plot Labels and Titles
PLOT_TITLE = None
X_LABEL = r"Li$_6$ Enrichment (%)"
Y_LABEL = "TBR"
LEGEND_LABEL = None

# 4. Visual Settings
MARKER_STYLE = '+'                 # 'o' for circles, 's' for squares, '^' for triangles
MARKER_SIZE = 5
...
def main():
    ...
    plt.errorbar(
        x, y,
        yerr=y_err,
        fmt=MARKER_STYLE + LINE_STYLE,  # Combines marker and line style
        markersize=MARKER_SIZE,  # <--- ADD THIS LINE HERE
        color=COLOR,
